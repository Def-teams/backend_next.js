  ...
  export async function POST(req: NextRequest) {
    try {
      ...
      const resetToken = jwt.sign({ userId: String(user.userId) }, secret, { expiresIn: '1h' });
      const encodedToken = encodeURIComponent(resetToken).replace(/\./g, '%2E');
      const resetLink = `${process.env.NEXT_PUBLIC_BASE_URL}/auth/email/reset_passwd?userId=${user.userId}&token=${encodedToken}`;
